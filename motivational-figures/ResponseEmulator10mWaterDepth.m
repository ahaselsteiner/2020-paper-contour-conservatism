classdef ResponseEmulator10mWaterDepth
    ...
    properties
        ...
        mu = @(v1hr, hs, tp) (tp >= sqrt(2 * pi .* hs ./ (9.81 .* 1/9.99))) .* ...
            (((5/6 * (v1hr <= 25) .* (3.2586e+06  .* v1hr + 7.1014e+07 ./ (1 + 0.040792 * (v1hr - 11.6).^2) - 7.1014e+07 ./ (1 + 0.040792 * (0 - 11.6).^2)) + ... 
            (v1hr > 25 ) .* (3.9 * 10^4  .* v1hr.^2)).^2.0 + ...
            (1/3 * (1 + (v1hr > 25) * 0.3) .* 7.4124e+06  .* hs .* (1 + 3.5301 .* exp(-0.32814 * abs(tp - 3))) ).^2.0).^(1/2.0));
                
        sigma = @(v1hr, hs, tp) 0 + (tp >= sqrt(2 * pi .* hs ./ (9.81 .* 1/9.99))) .* ...
            (((5/6 * (v1hr <= 25) .* (1.1639e+05 .* v1hr + 2.4543e+07  ./ (1 + 0.064 * (v1hr - 11.6).^2) +  -1.7917e+07./ (1 + 0.2 * (v1hr - 11.6).^2)) + ...
            (v1hr > 25) .* 4700 .* v1hr.^2).^2.0 + ...
            (1/3 * (1 + (v1hr > 25) * 0.3) .* 5.0422e+05 .* hs.^1.5 .* (1 + 11.909 .* exp(-0.6131 * abs(tp - 3))) ).^2.0).^(1/2.0));
